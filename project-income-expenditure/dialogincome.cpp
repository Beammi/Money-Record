    ...
    ui(new Ui::DialogIncome)
{
    ...
    connect(ui->moneyLineEdit,&QLineEdit::textChanged,this,&DialogIncome::on_moneyLineEdit_inputRejected); //connet signal to check the user input(money)
...
void DialogIncome::load()
{
    //load the data by using QSettings
    QSettings settings("Beam","MyApp"); //this is like a file name
    int listsize = settings.beginReadArray("Income1"); //create the name of array
    ...
    for (int i = 0;i < listsize; i++)
    {
        ...
        qInfo() << "load";
        ui->listWidget->addItem(settings.value("Income").toString()); //add item to listwidget

        allItemsinListwidget = settings.value("Income").toString(); //get each the data, and use for calculating
...
void DialogIncome::save()
{
    QSettings settings("Beam","MyApp");
    settings.clear(); //always clear, careful with redundant data
    settings.beginWriteArray("Income1");

    for(int i = 0; i < ui->listWidget->count(); i++)
    {
        settings.setArrayIndex(i);

        settings.setValue("Income", ui->listWidget->item(i)->text());
    }
    settings.endArray();
...
void DialogIncome::on_addPushBtn_clicked()
{
...
void DialogIncome::on_buttonBox_clicked(QAbstractButton *button)
{
...
void DialogIncome::on_removePushBtn_clicked()
{
...
void DialogIncome::on_pushButton_clicked()
{
    //push button clear
...
void DialogIncome::on_calculatePushBtn_clicked()
{
    double sum = 0;
    ...
    QMessageBox::information(this,"Calculate","Incomes = "+changeToString);


}

void DialogIncome::on_moneyLineEdit_inputRejected()
{
    bool ok;
    double test;
